...
function checkGeo() {
  ...
  if (geoPosition.init()) {
    $('#checking-area').html("<br /><br /><span class = \"loading-text\">Checking your location...</span>");
    geoPosition.getCurrentPosition(success_geo_callback, error_geo_callback, {enableHighAccuracy: true, timeout: 20000});
    
  } else {
    $('#checking-area').html("<br /><br /><span class = \"error-loading-text\">Looks like your browser doesn't support geolocaton detection.<br />Try updating!</span>");
  }
}
...
function success_geo_callback(geo) {
  ...
  //Print lat and long
  $('#checking-area').html("<br /><span class = \"intro-text\">Verifying your location...</span> <br /><br />");
  ...
  createLoadBar(function() { $('#verify-location-form').submit(); }, 2000, $('#checking-area'));
}
...
function error_geo_callback(geo) {
  ...
  $('#checking-area').html("<br /><br /><span class = \"error-loading-text\">" + errorMsg + "</span><br /><br /><button class = \"btn btn-large btn-info\" id = \"test_geo_again\" type = \"button\">Try Again</button> "); 
